...
function Banana() {
    ...
    async function getAllMenu() {
        ...
        setError(null);
        try {
            const response = await fetch(`https://forkify-api.herokuapp.com/api/v2/recipes?search=Banana%20Bread`);
            if (!response.ok) {
            ...
            if (data.results > 0 && data.data.recipes) {
                console.log('Recipes Found:', data.data.recipes); // Log the recipes
                setResults(data.data.recipes); // Store all recipes in the results state
                setAllResults(data.data.recipes); // Store all recipes in the allResults state
            } else {
                console.log('No recipes found');
                setResults([]); // Set results to an empty array if no recipes found
                setAllResults([]); // Set allResults to an empty array if no recipes found
            }
        } catch (err) {
